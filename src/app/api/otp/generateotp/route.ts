...
export async function POST(request: Request) {
  try {
    ...
    const SIMPLE_MAIL_TRANSFER_PROTOCOL_USERNAME =
      process.env.SIMPLE_MAIL_TRANSFER_PROTOCOL_USERNAME;
    const SIMPLE_MAIL_TRANSFER_PROTOCOL_PASSWORD =
      process.env.SIMPLE_MAIL_TRANSFER_PROTOCOL_PASSWORD;
    ...
    return Response.json(
      { message: "ส่งรหัสสำเร็จ", data: email },
      { status: 200 }
    );
  } catch (error: unknown) {
    return Response.json(
      { error: "ส่งรหัสล้มเหลว", data: error },
      { status: 500 }
    );
  }
}
