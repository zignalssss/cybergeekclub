...
export default function All_Detial_Budget({ typeBudget, detial_income, detial_support }: detial) {
  return (
    <div className="bg-[#1E1E1E] rounded-2xl w-full sm:w-3/4 lg:w-2/4 my-4 mx-2 flex flex-col items-center">
      ...
      <div className="flex flex-col sm:flex-row justify-around w-full h-full overflow-auto scroll-smooth scrollbar-hide">
        <div className="px-5 py-2 w-full sm:w-1/2">
          <h2 className="px-5 pt-4 font-kanit font-bold text-l">รายได้จากเงินรายได้</h2>
          {detial_income.map((element, index) => (
            <div key={index} className="bg-[#302E2E] w-full rounded-lg px-2 py-2 my-2">
              <h1>{element.key}</h1>
